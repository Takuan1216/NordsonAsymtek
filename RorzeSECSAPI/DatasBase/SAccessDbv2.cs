...
namespace Rorze.DB
{
    ...
    public class SAccessDb
    {
        ...
        public void CompactDB()
        {
            ...
            lock (this)
            {
                string compactFile = string.Format(@"D:\Scientech\{0}", Path.GetFileName(_strDbPath));
                if (File.Exists(compactFile)) File.Delete(compactFile);
                ...
                object[] oParams = new object[]
                {
                    ...
                    string.Format("Provider=Microsoft.Jet.OLEDB.4.0; Data Source=D:\\Scientech\\{0};Jet OLEDB:Engine Type=5;Jet OLEDB:Database Password={1}",Path.GetFileName(_strDbPath),_strPassword)
                };
                //委派方法
                ...
                File.Move(string.Format(@"D:\Scientech\{0}", Path.GetFileName(_strDbPath)), _strDbPath);
                //重新open Db
                this.Open();
            }
            IsCompacting = false;
        ...
        public static void ExportXML(DataTable table, string path, bool bAppend = false)
        {
            if (table.Rows.Count <= 0)
            {
                SLogger.GetLogger("DataBase").WriteLog("[Export] export csv file failue. data table is empty.");
