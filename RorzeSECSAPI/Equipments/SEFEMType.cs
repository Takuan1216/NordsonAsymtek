...
namespace Rorze.Equipments
{
  public abstract class SEFEMType: SEFEM
  {
        ...
        public Dictionary<int, SLoadport> GetPort { get { return _loadportlist; } }
        SInterruptOneThreadINT _exeAutoClamp;

        public SEFEMType(string name,SocketControl control,int LoadportMax) :
            base(name, control)
        {
            _loadportlist = new Dictionary<int, SLoadport>();
            for (int i = 0; i < LoadportMax; i++)
                _loadportlist.Add(i + 1, new SLoadport(i + 1));
        ...
        public virtual void WaferSelect(int portID, string CJID, string PJID, string slotmap, string ProcessMode, string RecipeName)
        {
            SendOrder(_dicCmdsTable[EFEMTypeCommand.WaferSelect], portID.ToString(), CJID, PJID, slotmap, ProcessMode, RecipeName);
        ...
        public override bool ParseEventMsg(EFEMFrame frame)
        {
            ...
            switch (Event)
            {
                ...
                case EFEMTypeEvent.WfTransState:
                    ...
                    string UnitPos = frame.Parameter.Split(',')[1];
                    int slot = Convert.ToInt16(frame.Parameter.Split(',')[2]);
                    string WaferID = frame.Parameter.Split(',')[3];
                    if (OnWaferTranfer != null)
                        OnWaferTranfer(this, new EFEMWaferTranferEventArgs(IsRobotTake, UnitPos, WaferID, slot));
                    return true;
            }
            return false;
        }
