...
namespace Wid110LibUser
{
    public class Wid110Lib : IDisposable
    {
        ...
        [DllImport("wid110Lib.dll")]
        private static extern int FuncProcessReadConfig(IntPtr objptr,
                                                    string cfgName);
        ...
        private static extern int FuncSaveJob(IntPtr objptr,
                                                ...
                                                string cpFilename);
        [DllImport("wid110Lib.dll")]
        private static extern int FuncSaveConfig(IntPtr objptr,
                                                string cpFilePath,
                                                string cpFilename,
                                                string cfgName);
        ...
        [DllImport("wid110Lib.dll")]
        private static extern int FuncConfigureActivate(IntPtr objptr, string cName);
        ...
        [DllImport("wid110Lib.dll")]
        private static extern int FuncGetReaderInfo(IntPtr objptr,
                                                    StringBuilder cInfo,
                                                    int type,
                                                    int nMaxLen);
        ...
        /// Get reader version.
        /// </summary>
        /// <param name="v"> WID110 reader version.</param>
        // ====================================================================
        public string FGetReaderInfo(int type)
        {
            try
            {
                version = "";
                StringBuilder sb = new StringBuilder("", Wid110LibConst.versLenCS);
                if (HandleDLLStatus(FuncGetReaderInfo(dll, sb, type, Wid110LibConst.versLenC)))
                {
                    version = sb.ToString();
                }
            }
        ...
        // ====================================================================
        /// <summary>
        /// Perform a process read only with specified configuration
        /// </summary>
        /// <return> true if done.</return>
        // ====================================================================
        public bool FProcessReadConfig(string cfgName)
        {
            try
            {
                if (HandleDLLStatus(FuncProcessReadConfig(dll, cfgName))) return true;
            }
            catch (Exception e) { lastExcp = e.ToString(); }
            return false;
        }
                                      ...
                                      int type)
        {
            try
            {
                if (HandleDLLStatus(FuncProcessGetImage(dll, name, type))) 
                    return true;
            }
            catch (Exception e) { lastExcp = e.ToString(); }
            return false;
        ...
        /// <return>            true if OK.</return>
        // ====================================================================
        public bool FSaveConfig(string jobFilePath, string cfgName)
        {
            string path = Path.GetDirectoryName(jobFilePath);
            string file = Path.GetFileName(jobFilePath);
            try
            {
                if (HandleDLLStatus(FuncSaveConfig(dll, path, file, cfgName))) return true;
            }
            catch (Exception e) { lastExcp = e.ToString(); }
            return false;
        }
        ...
        public bool FLoadRecipes(string jobFilePath)
        {
            try
            {
                ...
                if (HandleDLLStatus(FuncLoadRecipes(dll, path, file)))
                {
                    DateTime t2 = DateTime.Now;
                    TimeSpan ts = t1.Subtract(t2);
                    double dr = (t2 - t1).TotalMilliseconds;
                    return true;
                }
            }
            catch (Exception e) { lastExcp = e.ToString(); }
        ...
        // ====================================================================
        /// <summary>
        /// Activate configuration by the name.
        /// </summary>
        /// <param name="configName">Configuration Name</param>
        /// <returns>true if no error</returns>
        // ====================================================================
        public bool FConfigureActivate(string configName)
        {
            try
            {
                if (HandleDLLStatus(FuncConfigureActivate(dll, configName))) return true;
            }
            catch (Exception e) { lastExcp = e.ToString(); }
            return false;
        }
