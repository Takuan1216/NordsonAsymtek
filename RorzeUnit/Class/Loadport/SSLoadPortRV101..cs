...
{
    public class SSLoadPortRV101 : SSLoadportParents
    {
        #region =========================== private ============================================
        ...
        #endregion
        //==============================================================================
        public SSLoadPortRV101(I_E84 e84, string strIP, int nPortID, int nBodyNo, bool bDisable, bool bSimulate, int[] nTrbMapStgNo0to399,
        ...
        private void AnalysisStatus(string strFrame)
        {
            ...
            {
                _logger.WriteLog("[STG{0}] the format of STAT frame has error, '/' not found! [{1}]", this.BodyNo, strFrame);
                return;
            }
        ...
        private void AnalysisGPIO(string strFrame)
        {
            ...
            if (OnIOChange != null)
                OnIOChange(this, new RorzenumLoadportIOChengeEventArgs(_gpio));
        ...
        private void AnalysisGPOS(string strFrame)
        {
            try
            {
                ...
                /*
                if (strFrame.Contains("RCA1"))
                {
                    //_Mappingdata.AddMappingRowData()
                    int Value =int.Parse(strFrame.Split('+')[1]);
                    _Mappingdata.AddMappingRowData(Value);  
                }
                else
                {
                */
                _Yaxispos = (enumLoadPortPos)int.Parse(strFrame.Split('/')[0]);
                _Zaxispos = (enumLoadPortPos)int.Parse(strFrame.Split('/')[1]);
                //}
            }
            catch (Exception ex)
            {
        ...
        private void AnalysisGWID(string strFrame)
        {
            try
            {
        ...
        protected override void ExeClamp1()
        {
            try
            {
                ...
                OnClmp1Complete?.Invoke(this, new LoadPortEventArgs(_MappingData, BodyNo, true));
            }
            catch (SException ex)
            {
                WriteLog("<<Alarm>> ExeClamp1:" + ex);
            }
            catch (Exception ex)
            {
                WriteLog("<<Alarm>> ExeClamp1:" + ex);
        ...
        protected override void ExeUClamp1()
        {
            try
            {
                ...
                OnUclm1Complete?.Invoke(this, new LoadPortEventArgs(_MappingData, BodyNo, true));
            }
            catch (SException ex)
            {
        ...
        protected override void ExeWMAP()
        {
            try
            {
                ...
                //  通知外界完成
                OnMappingComplete?.Invoke(this, new LoadPortEventArgs(_MappingData, BodyNo, true));
            }
            catch (SException ex)
            {
        ...
        protected override void ExeGetData()
        {
            try
            {
                ...
                //  DPRM
                for (int nDprm = 0; nDprm < 16; nDprm++)
                {
                    this.GetDprmW(3000, nDprm);
                    this._sDPRMData[nDprm] = m_strDprm;
                }
        ...
        #endregion
        //==============================================================================
        private void SSLoadPort_OnIOChange(object sender, RorzenumLoadportIOChengeEventArgs e)
        {
            try
            {
                ...
                {
                    if (e.Frame.GetDIList[LoadPortGPIO.LoadPortDI._DIPresenceright] &&     //have Foup 
                        e.Frame.GetDIList[LoadPortGPIO.LoadPortDI._DIPresencemiddle] &&
                        e.Frame.GetDIList[LoadPortGPIO.LoadPortDI._DIPresenceleft] &&
                        e.Frame.GetDIList[LoadPortGPIO.LoadPortDI._DIPresence]
                        && !FoupExist)
        ...
        public override void OrgnW(int nTimeout, int nVariable = 0)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Orgn].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send Orgn command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Orgn]));
        ...
        public override void ClmpW(int nTimeout, int nVariable = 0)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Clamp].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send CLMP command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Clamp]));
        ...
        public override void UclmW(int nTimeout, int nVariable = 0)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.UnClamp].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send UCLM command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.UnClamp]));
        ...
        public override void WmapW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Mapping].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send WMAP command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Mapping]));
                if (_signalAck[enumLoadPortCommand.Mapping].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send WMAP command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.Mapping]));
            }
            else
            {
        ...
        public override void LoadW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.E84Load].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send Load command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.E84Load]));
        ...
        public override void UnldW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.E84UnLoad].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send UCLM command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.E84UnLoad]));
        ...
        public override void EventW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.SetEvent].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Event command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetEvent]));
                if (_signalAck[enumLoadPortCommand.SetEvent].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Event command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetEvent]));
            }
            else
            {
        ...
        public override void ResetW(int nTimeout, int nReset = 0)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Reset].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Reset command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Reset]));
                if (_signalAck[enumLoadPortCommand.Reset].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Reset command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.Reset]));
            }
            else
            {
        ...
        public override void InitW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Initialize].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.InitialFailure, string.Format("Send Initialize command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Initialize]));
        ...
        public override void StopW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Stop].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Stop command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Stop]));
                if (_signalAck[enumLoadPortCommand.Stop].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Stop command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.Stop]));
            }
            else
            {
        ...
        public override void PausW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Pause].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Pause command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Pause]));
                if (_signalAck[enumLoadPortCommand.Pause].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Pause command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.Pause]));
            }
            else
            {
        ...
        public override void ModeW(int nTimeout, int nMode)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Mode].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Mode command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Mode]));
                if (_signalAck[enumLoadPortCommand.Mode].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send Mode command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.Mode]));
            }
            else
            {
        ...
        public override void WtdtW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Wtdt].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send WTDT command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Wtdt]));
                if (_signalAck[enumLoadPortCommand.Wtdt].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send WTDT command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.Wtdt]));
            }
            else
            {
        ...
        public override void SspdW(int nTimeout, int nVariable)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.Speed].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send SSPD command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.Speed]));
        ...
        public override void GpioW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetIO].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GPIO command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetIO]));
                if (_signalAck[enumLoadPortCommand.GetIO].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GPIO command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetIO]));
            }
            else
            {
        ...
        public override void GmapW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetMappingData].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send WMAP command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetMappingData]));
                if (_signalAck[enumLoadPortCommand.GetMappingData].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send WMAP command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetMappingData]));
            }
            else
            {
                SWafer waferShow;
        ...
        public override void Rca2W(int nTimeout, int nVariable)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetRAC2].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send RAC2 command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetRAC2]));
                if (_signalAck[enumLoadPortCommand.GetRAC2].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send RAC2 command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetRAC2]));
            }
            else
            {
        ...
        public override void GverW(int nTimeout)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetVersion].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GVER command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetVersion]));
                if (_signalAck[enumLoadPortCommand.GetVersion].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GVER command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetVersion]));
            }
            else
            {
        ...
        public override void StimW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.SetDateTime].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send STIM command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetDateTime]));
                if (_signalAck[enumLoadPortCommand.SetDateTime].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send STIM command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetDateTime]));
            }
            _signalSubSequence.Set();
        }
        #endregion         
        ...
        public override void GposW(int nTimeout)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetPos].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GPOS command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetPos]));
                if (_signalAck[enumLoadPortCommand.GetPos].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GPOS command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetPos]));
            }
            else
            {
        ...
        public override void GwidW(int nTimeout)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetType].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GWID command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetType]));
                if (_signalAck[enumLoadPortCommand.GetType].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GWID command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetType]));
            }
            else
            {
        ...
        public override void SwidW(int nTimeout, string strId)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.SetType].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send SWID command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetType]));
                if (_signalAck[enumLoadPortCommand.SetType].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send SWID command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetType]));
            }
            else
            {
            ...
            if (!Simulate)
            {
                ...
                if (!Simulate)
                {
                    if (!_signalAck[enumLoadPortCommand.YaxStep].WaitOne(nTimeout))
                        throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send YAX1 STEP command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.ZaxStep]));
                    if (_signalAck[enumLoadPortCommand.YaxStep].bAbnormalTerminal)
                        throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send YAX1 STEP command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.ZaxStep]));
                }
            }
            else
            {
        ...
        public override void ZaxStepW(int nTimeout, string strStep)
        {
            ...
            if (!Simulate)
            {
                if (!_signalAck[enumLoadPortCommand.ZaxStep].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GetPos command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.ZaxStep]));
                if (_signalAck[enumLoadPortCommand.ZaxStep].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send GetPos command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.ZaxStep]));
            }
            _signalSubSequence.Set();
        }
        #endregion 
        ...
        public override void YaxHomeW(int nTimeout, int nHome)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.YaxHome].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send YAX1 HOME command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.YaxHome]));
                if (_signalAck[enumLoadPortCommand.YaxHome].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send YAX1 HOME command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.YaxHome]));
            }
            else
            {
        ...
        public override void ZaxHomeW(int nTimeout, int nHome)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.ZaxHome].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send ZAX1 HOME command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.ZaxHome]));
                if (_signalAck[enumLoadPortCommand.ZaxHome].bAbnormalTerminal)
                    throw new SException((int)enumLoadPortError.SendCommandFailure, string.Format("Send ZAX1 HOME command is Canecl. [{0}]", _dicCmdsTable[enumLoadPortCommand.ZaxHome]));
            }
            else
            {
        ...
        public override void GetDprmW(int nTimeout, int p1)
        {
            ...
            if (!Simulate || Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetDPRM].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send OPR command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetDPRM]));
        ...
        public override void GetDmprW(int nTimeout)
        {
            ...
            if (Connected)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetDMPR].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send OPR command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetDMPR]));
        ...
        public override void SetDmprW(int nTimeout, int p1, string strData)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.SetDMPR].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send OPR command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetDMPR]));
        ...
        public override void GetDcstW(int nTimeout)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.GetDCST].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send OPR command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.GetDCST]));
        ...
        public override void SetDCSTW(int nTimeout, string strData)
        {
            ...
            if (!Simulate)
            {
                ...
                if (!_signalAck[enumLoadPortCommand.SetDCST].WaitOne(nTimeout))
                    throw new SException((int)enumLoadPortError.AckTimeout, string.Format("Send OPR command and wait Ack was timeout. [{0}]", _dicCmdsTable[enumLoadPortCommand.SetDCST]));
