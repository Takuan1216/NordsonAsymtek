...
namespace RorzeUnit.Class.Agito
{
    class CINIFile
    {
        ...
        [DllImport("kernel32", CharSet = CharSet.Unicode, SetLastError = true)]
        private static extern int GetPrivateProfileString(string sectionName, string keyName, string defaultReturnString, byte[] returnBuffer, int returnStringLength, string filePath);
        ...
        public bool WriteIni(string strSection, string strKey, int nValue)
        {
            return WritePrivateProfileString(strSection, strKey, nValue.ToString(), this.m_strFile);
        }
        public bool WriteIni(string strSection, string strKey, double nValue)
        {
            return WritePrivateProfileString(strSection, strKey, nValue.ToString(), this.m_strFile);
        }
        public bool WriteIni(string strSection, string strKey, bool bBOOL)
        {
            string strValue = "";
            strValue = (bBOOL) ? "1" : "0";
            return WritePrivateProfileString(strSection, strKey, strValue, this.m_strFile);
        }
        public bool WriteIni(string strSection, string strKey, string strValue)
        {
            return WritePrivateProfileString(strSection, strKey, strValue, this.m_strFile);
        }
        ...
        public int GetIni(string strSection, string strKey, int nDefautl = 0)
        {
            int nValue = 0;
            StringBuilder temp = new StringBuilder(255);
        ...
        public bool GetIni(string strSection, string strKey, bool bDefault = false)
        {
            bool bValue = false;
        ...
        public double GetIni(string strSection, string strKey, double dDefault = 0.0)
        {
            double dValue;
            StringBuilder temp = new StringBuilder(255);
            GetPrivateProfileString(strSection, strKey, "NOTFIND", temp, 255, this.m_strFile);
        ...
        public List<string> ReadSections()
        {
            ...
            int nLength = GetPrivateProfileString(null, null, null, buffer, buffer.Length, m_strFile);

            string str1 = System.Text.Encoding.UTF8.GetString(buffer, 0, nLength * 2).Replace("\0\0", "@");//@分隔符號
            string str2 = str1.Replace("\0", "");
            string[] rs = str2.Split(new string[] { "@" }, StringSplitOptions.RemoveEmptyEntries);
            return rs.ToList();
        }
